                ...
                <div x-data="{ open: @json(request()->routeIs('admin.products.*') || request()->routeIs('admin.categories.*') || request()->routeIs('admin.features.*')) }">
                    ...
                    <div x-show="open" x-collapse.duration.200ms>
                        <div class="mt-1 space-y-1 pl-4">
                            <a href="{{ route('admin.categories.index') }}" class="flex items-center gap-3 pl-6 pr-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 @if(request()->routeIs('admin.categories.*')) bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 shadow-sm @else text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white @endif">
                                <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                </svg>
                                Categories
                            </a>
                            <a href="{{ route('admin.features.index') }}" class="flex items-center gap-3 pl-6 pr-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 @if(request()->routeIs('admin.features.*')) bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 shadow-sm @else text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white @endif">
                                <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <aside id="mobile-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shadow-2xl -translate-x-full transition-transform duration-300 lg:hidden">
            ...
            <nav class="py-4 px-3 space-y-1">
                ...
                <div x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition text-left">
                        <span class="flex-1">Products</span>
                        <svg class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <div x-show="open" x-collapse.duration.200ms class="pl-6 mt-1 space-y-1">
